...
public class QQQuoteFetcher extends BaseQuoteFetcher {
    private static final String TAG = "QQQuoteFetcher";
    ...
    public StockDetail fetch(String quote) throws DownloadException, ParseException {
        ...
        try {
            openReq.setHeader("Referer", "http://QQ.on.cc/");
    ...
    public String getUrl(String quote) {
        return String.format("http://QQ.on.cc/info/liveinfo_quote.html?symbol=%s", quote);
    }

    private String getOpenUrl(String quote) {
        SimpleDateFormat formatter = new SimpleDateFormat(DATE_PARAM_FORMAT);
        Calendar cal = Calendar.getInstance();
        return String.format("http://QQ.on.cc/js/daily/quote/%s_d.js?t=%s",
                quote, formatter.format(cal.getTime()));
    }

    private String getUpdateUrl(String quote) {
        return String.format("http://QQ.on.cc/js/real/quote/%s_r.js", quote);
    }
